          ...
          <div className="h-full hidden md:flex justify-center items-center space-x-4 w-10/12">
            {links.map((link, i) => (
              <Link
                to={link.url}
                key={i}
                className="text-white group py-16 hover:text-custom-yellow"
                onClick={handleLinkClick}
              >
                <span className="border-e-2 pr-4 group-last:border-e-0 border-custom-yellow">
                  {link.title}
                </span>
                  ...
                  <div className="flex flex-col h-60 w-5/12">
                    {link.additionalData.map((link, i) => (
                      <Link
                        to={link.url}
                        key={i}
                        className="text-white hover:underline underline-offset-8 decoration-custom-yellow-dark rounded-sm border-maroon-50 p-2"
                        onClick={handleLinkClick}
                      >
                        {link.title}
                      </Link>
            ...
            <button
              onClick={() => setPrashadModalOpen(true)}
              className="text-white py-16 hover:text-custom-yellow"
            >
          ...
          <div className="md:hidden flex flex-col gap-3 bg-custom-maroon p-4 border-b border-custom-yellow-dark w-[calc(90vw)]">
            {links.map((link, i) => (
              <Link
                to={link.url}
                key={i}
                className="text-white group hover:text-custom-yellow pr-4 "
                onClick={toggleMenu}
              >
                {link.title}
                <ul className="hidden group-hover:flex flex-col absolute w-auto bg-custom-maroon p-2 z-50">
                  {link.additionalData.map((link, i) => (
                    <Link
                      ...
                      onClick={toggleMenu}
                      key={i}
                      className="text-white hover:bg-custom-yellow rounded-sm border-maroon-50 p-2"
                    >
                      {link.title}
                    </Link>
                  ))}
            ...
            <button
              onClick={() => setPrashadModalOpen(true)}
              className="text-white hover:text-custom-yellow self-start"
            >
              Prashad Corner
            </button>
            {/* Added by Abhishek Pathak */}
            <div className="w-full relative md:w-2/12 md:flex justify-center">
              <button
                className="bg-custom-yellow text-white p-1 rounded-md"
                onClick={appointmentHandler}
              >
          ...
          >
            <div className="bg-white p-6 rounded-lg w-96">
              <h2 className="text-xl font-bold text-center mb-4">
                Prashad Shipment Details
              </h2>
              <form onSubmit={handlePrashadSubmit} className="space-y-3">
                <input
                  type="text"
                  name="name"
                  value={prashadData.name}
                  onChange={handlePrashadChange}
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => setPrashadModalOpen(false)}
